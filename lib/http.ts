...
export async function readJson(request: Request, maxBytes = MAX_BODY_BYTES): Promise<unknown> {
  ...
  if (Number(request.headers.get("content-length")) > maxBytes) throw new AppError("BODY_TOO_LARGE", "请求资料或参考图片过大。", 413);
  ...
  try {
    while (true) {
      ...
      if (size > maxBytes) { await reader.cancel(); throw new AppError("BODY_TOO_LARGE", "请求资料或参考图片过大。", 413); }
      chunks.push(value);
    }
